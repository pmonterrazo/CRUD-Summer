...
function getItensBD() {
  return JSON.parse(localStorage.getItem('dbentradapaciente')) ?? [];
}


function setItensBD() {
  localStorage.setItem('dbentradapaciente', JSON.stringify(itens));
}
...
btnSalvar.onclick = e => {
  if (
    sPaciente.selectedOptions.length === 0 ||
    sMedico.selectedOptions.length === 0 ||
    sQuarto.selectedOptions.length === 0 ||
    sMedicamento.selectedOptions.length === 0
  ) {
    return;
  }
  ...
  const pacienteValue = sPaciente.selectedOptions[0].value;
  const medicoValue = sMedico.selectedOptions[0].value;
  const quartoValue = sQuarto.selectedOptions[0].value;
  const medicamentoValue = sMedicamento.selectedOptions[0].value;

  console.log(medicoValue)
...
function populateSelects() {
  ...
  medicamentos.forEach(medicamento => {
    const option = document.createElement('option');
    option.value = medicamento.desccom;
